...
inline void Array<T>::copy(const Array<T>& src, const size_t srcStartIndex, Array<T>& dest, const size_t destStartIndex, const size_t length)
{
	if (&src == &dest && destStartIndex + length > srcStartIndex && destStartIndex < srcStartIndex + length)
		memmove(dest.reinterpret_cast<byte*>(this->pointer_) + destStartIndex * sizeof(T),
			src.reinterpret_cast<byte*>(this->pointer_) + destStartIndex * sizeof(T), length);
	else
		memcpy(dest.reinterpret_cast<byte*>(this->pointer_) + destStartIndex * sizeof(T),
			src.reinterpret_cast<byte*>(this->pointer_) + destStartIndex * sizeof(T), length);
}
